...
router.get("/me", authSuperUser, async (req, res) => {
  const superUser = await SuperUser.findById(req.superUser._id).select(
    "-Password"
  );
  res.send(superUser);
});
...
router.get("/:id", authSuperUser, async (req, res) => {
  ...
  if (!superUser)
    return res.status(400).send("No SuperUser were found with the given id");
...
router.put("/:id", authSuperUser, async (req, res) => {
  ...
  const superUser = await SuperUser.findByIdAndUpdate(
    req.params.id,
    {
      EmailID: req.body.EmailID,
      FirstName: req.body.FirstName,
