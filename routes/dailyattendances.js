...
router.post("/", [authDriver], async (req, res) => {
  const { error } = validate(req.body);

  if (!mongoose.Types.ObjectId.isValid(req.body.DriverID))
...
router.get("/:id", [authParent], async (req, res) => {
  ...
  if (!dailyAttendance)
    return res
      .status(400)
      .send("No DailyAttendance were found with the given id");

  res.send(dailyAttendance);
});

router.put("/:id", [authDriver], async (req, res) => {
  const { error } = validate(req.body);
  ...
  const dailyAttendance = await DailyAttendance.findByIdAndUpdate(
    req.params.id,
    {
      Session: req.body.Session,
      Status: req.body.Status,
  ...
  if (!dailyAttendance)
    return res
      .status(400)
      .send("No Daily Attendance were found with the given id");
