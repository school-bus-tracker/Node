...
router.get("/me", authSchoolAdmin, async (req, res) => {
  const schoolAdmin = await SchoolAdmin.findById(req.schoolAdmin._id).select(
    "-Password"
  );
...
router.get("/:id", [authParent], async (req, res) => {
  ...
  if (!schoolAdmin)
    return res.status(400).send("No SchoolAdmin were found with the given id");
...
router.put("/:id", authSuperUser, async (req, res) => {
  ...
  if (!schoolAdmin)
    return res.status(400).send("No School Admin were found with the given id");
